...
export async function sendTelegramChunks(bot: TelegramBotApi, chatId: number, markdown: string): Promise<void> {
  const chunks = renderTelegramMarkdownChunks(markdown);
  for (const [index, chunk] of chunks.entries()) {
    const messageId = await bot.sendMessage({ chatId, text: chunk });
    log(`sent telegram chunk ${index + 1}/${chunks.length} messageId=${messageId ?? 'unknown'} chars=${chunk.length}`);
  }
}
